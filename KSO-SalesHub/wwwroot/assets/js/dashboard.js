        ...
        var getData = $.ajax({
            ...
            success: function (data) {
                ...
                if (data != null && data.length > 0) {
                    ...
                    // Initialize index to loop through colors
                    var colorIndex = 0;

                    // Loop through the data to dynamically populate the labels and data
                    data.forEach(function (item) {
                        labels.push(item.category); // Push the category name to labels array
                        dataValues.push(item.count); // Push the count to data array

                        // Assign a color to the current category (you can customize the color assignment logic here)
                        if (colorIndex < categoryColors.length) {
                            categoryColors.push(categoryColors[colorIndex % categoryColors.length]); // Prevent overflow
                        }
                        colorIndex++;
                    });
                    ...
                    new Chart($('#chartjsPieBosHelpdesk'), {
                        ...
                        data: {
                            ...
                            datasets: [{
                                label: "Population (millions)",
                                backgroundColor: categoryColors.slice(0, labels.length), // Dynamically assign colors based on number of labels
                                borderColor: colors.cardBg,
                                data: dataValues, // Dynamically generated data
                                datalabels: {
                                    anchor: 'center',
                                }
                            }]
