...
var last90days = $('#pick-date').val();
function resetDays() {
	$('#pick-date').val(last90days);
}
...
! function (a) {
	...
	c.prototype.set = function (a, b, c) {
		...
		return e.node("div", (c.selectYears ? q() + p() : p() + q()) + o() + o(1), c.klass.header) + e.node("table", n + e.node("tbody", e.group({
			...
			controls: b.$node[0].id
		})) + e.node("button", c.clear, c.klass.buttonClear, "type=button data-close=true" + " " + "onclick=resetDays()" + " " + e.ariaAttr({
			controls: b.$node[0].id
